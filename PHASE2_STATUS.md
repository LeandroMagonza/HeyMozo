...
- **Verificado**: `npm run migrate` OK (columnas creadas vía sync + migration idempotente), `npm run build` compila sin errores nuevos. **Smoke automatizado a nivel servicio OK (22/22)**: validar transfer (paid + auto-cierre), rechazar (failed, sesión queda), acuse efectivo (acknowledge idempotente), scan-only filtrado, liberar mesa (cierra + cancela pedidos pending + VACATE + releaseReason/closedBy). Corrido contra la DB de dev con setup/teardown aislado. **Falta**: abrir PR (opcional: pasada visual en navegador con `npm run dev`).
