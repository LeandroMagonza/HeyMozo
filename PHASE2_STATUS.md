...
**5.8 — `feature/phase2-cajashell-acciones`** (desde `master` @ 62dc205). Implementado en local, **pendiente smoke E2E + PR**.
...
- **Finding pendiente de decisión (NO bug de 5.8, viene de 5.4–5.6)**: cuando una sesión se **auto-cierra por pago completo** (validate/collect/MP webhook), los pedidos `pending` quedan colgados como card "Nuevo Pedido" en el Piso (a diferencia de `releaseTable`, que sí los cancela). Es ambiguo: un pedido pending podría ser comida en preparación que el cliente pagó. Opciones: (a) dejar como está (la card refleja un pedido no entregado, el mozo marca LISTO al entregar), (b) cancelar pending al auto-cerrar, (c) marcarlos ready. A decidir con Tomas — probablemente Sprint 6 (refinamientos de cierre de mesa).
- **Falta**: solo abrir PR (push pendiente del OK de Tomas).
