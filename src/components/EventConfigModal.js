...
const EventConfigModal = ({
  ...
  console.log("EventConfigModal rendered with props:", {
    isOpen,
    resourceType,
    resourceId,
    companyId,
  ...
  const loadEventsWithConfiguration = useCallback(async () => {
    console.log("loadEventsWithConfiguration called with:", {
      resourceType,
      resourceId,
      companyId,
      branchId,
    ...
    try {
      ...
      const response = await fetch(
        `/api/resources/${resourceType}/${resourceId}/events/resolved?${queryParams}`,
        {
          headers: {
      ...
      if (response.ok) {
        ...
        console.error(
          "Failed to load events with configuration:",
          response.status
        );
      }
    } catch (error) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/resources/${resourceType}/${resourceId}/events/config`,
        {
          method: "POST",
          headers: {
  ...
  const handleReset = async () => {
    if (
      !window.confirm(
        "¿Estás seguro de que quieres restablecer todas las configuraciones para este recurso? Esto heredará las configuraciones del nivel superior."
      )
    ) {
      return;
    }
  ...
  const handleCreateEvent = async () => {
    try {
      // Determine the endpoint based on resourceType
      let endpoint;
      if (resourceType === "company") {
        endpoint = `/api/companies/${companyId}/event-types`;
      } else if (resourceType === "branch") {
        endpoint = `/api/branches/${branchId}/event-types`;
      } else {
        alert(
          "Los eventos personalizados solo pueden crearse a nivel de compañía o sucursal"
        );
        return;
      }
      ...
      if (response.ok) {
        ...
        alert(
          "Error creando el tipo de evento: " +
  ...
  const handleDeleteEvent = async (eventId, event) => {
    if (event.systemEventType) {
      alert(
        "Los eventos del sistema no pueden ser eliminados. Solo pueden ser renombrados."
      );
      return;
    }

    // Allow deletion from company level, or from branch level if it's a branch-specific event
    const canDelete =
      resourceType === "company" ||
      (resourceType === "branch" && event.branchId);

    if (!canDelete) {
      alert(
        "Los eventos personalizados solo pueden eliminarse desde su nivel de creación (compañía o sucursal)"
      );
      return;
    }
    ...
    if (
      !window.confirm(
        `¿Estás seguro de que quieres eliminar el evento "${event.eventName}"?`
      )
    ) {
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(
          "Error eliminando el tipo de evento: " +
            (error.error || "Error desconocido")
        );
      }
    } catch (error) {
                              ...
                              <div className="field-group">
                                ...
                                {isFieldOverridden(event, "eventName") && (
                                  <button
                                    ...
                                    onClick={() =>
                                      handleFieldReset(event.id, "eventName")
                                    }
                              ...
                              <div className="field-group">
                                ...
                                {isFieldOverridden(event, "stateName") && (
                                  <button
                                    className="reset-field-btn"
                                    onClick={() =>
                                      handleFieldReset(event.id, "stateName")
                                    }
                                    title="Restablecer al valor heredado"
                                  >
                                    <FaUndo />
                              ...
                              <div className="color-field">
                                ...
                                {isFieldOverridden(event, "userColor") && (
                                  <button
                                    className="reset-field-btn"
                                    onClick={() =>
                                      handleFieldReset(event.id, "userColor")
                                    }
                              ...
                              <div className="color-field">
                                ...
                                {isFieldOverridden(event, "adminColor") && (
                                  <button
                                    className="reset-field-btn"
                                    onClick={() =>
                                      handleFieldReset(event.id, "adminColor")
                                    }
                              ...
                              <div className="icon-field">
                                ...
                                >
                                  {getEffectiveValue(event, "userIcon") ? (
                                    ...
                                    <span className="no-icon-placeholder">
                                      📷
                                    </span>
                                  )}
                                </div>
                                {isFieldOverridden(event, "userIcon") && (
                                  <button
                                    className="reset-field-btn"
                                    onClick={() =>
                                      handleFieldReset(event.id, "userIcon")
                                    }
                      ...
                      <div className="event-controls">
                        <div className="inheritance-info">
                          <span
                            ...
                            title={getInheritanceText(event)}
                          >
                          ...
                          <div className="enabled-field">
                            ...
                            {isFieldOverridden(event, "enabled") && (
                              <button
                                className="reset-field-btn"
                                onClick={() =>
                                  handleFieldReset(event.id, "enabled")
                                }
                ...
                <div className="icon-selection-group">
                  ...
                  {newEvent.userIcon && (
                    <button
                      ...
                      onClick={() =>
                        setNewEvent((prev) => ({ ...prev, userIcon: null }))
                      }
                    >
                      <FaTrash /> Quitar
                    </button>
