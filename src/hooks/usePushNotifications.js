...
export function usePushNotifications(branchId = null) {
  ...
  // Wait for existing service worker registration and check existing subscription
  useEffect(() => {
    if (!isSupported) return;

    async function init() {
      try {
        // Use the existing SW registration from index.js (don't re-register)
  ...
  const sendTest = useCallback(async () => {
    try {
      ...
      setError('Error al enviar notificacion de prueba');
      return null;
    }
  }, []);
